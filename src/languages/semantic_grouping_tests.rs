...
"#;
    ...
    let tree = parser.parse("rust", rust_code).expect("Failed to parse Rust code");
    ...
    let group = rust_lang.find_semantic_group(&tree, function_node)
        .expect("Should find semantic group");
    
    println!("Function node: {:?}", function_node.kind());
    println!("Function text: {}", &rust_code[function_node.start_byte()..function_node.end_byte()]);
    ...
    let range_with_attrs = rust_lang.calculate_replacement_range(&tree, function_node, replacement_with_attributes, rust_code)
        .expect("Should calculate range with attrs");
    let range_without_attrs = rust_lang.calculate_replacement_range(&tree, function_node, replacement_without_attributes, rust_code)
    ...
    if group.has_preceding_elements() {
        ...
        assert_eq!(range_with_attrs, group_range, "With attributes should replace whole group");
        assert_eq!(range_without_attrs, (function_node.start_byte(), function_node.end_byte()), 
