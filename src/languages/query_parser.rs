use crate::languages::traits::{LanguageParser, LanguageQueries, NodeTypeInfo};
...
impl LanguageParser for QueryBasedParser {
    fn find_by_name<'a>(
        ...
        if !node_info.supports_search_by_name {
            return Err(anyhow!(
                "Node type {} doesn't support name-based search",
                node_type
            ));
        }
        ...
        let query_text = if node_info.fields.contains(&"name".to_string()) {
            ...
            return Err(anyhow!(
                "Node type {} has no searchable name field",
                node_type
            ));
        };
