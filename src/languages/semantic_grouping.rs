...
impl<'a> SemanticGroup<'a> {
    ...
    pub fn byte_range(&self) -> (usize, usize) {
        let start = self.preceding_nodes
            .first()
            .map(|n| n.start_byte())
            .unwrap_or(self.primary_node.start_byte());
            
        let end = self.following_nodes
            .last()
            .map(|n| n.end_byte())
...
pub trait SemanticGrouping {
    ...
    fn find_semantic_group<'a>(&self, tree: &'a Tree, node: Node<'a>) -> Result<SemanticGroup<'a>> {
        ...
        let applicable_rule = rules.iter()
    ...
    fn find_preceding_nodes<'a>(&self, _tree: &'a Tree, target_node: Node<'a>, rule: &GroupingRule) -> Result<Vec<Node<'a>>> {
        ...
        while cursor.goto_previous_sibling() {
            ...
            let is_matching = rule.preceding_node_types.contains(&current_node.kind().to_string());
    ...
    fn find_following_nodes<'a>(&self, _tree: &'a Tree, target_node: Node<'a>, rule: &GroupingRule) -> Result<Vec<Node<'a>>> {
        let Some(parent) = target_node.parent() else {
            return Ok(Vec::new());
        };
        ...
        while cursor.goto_next_sibling() {
            let current_node = cursor.node();
            let is_matching = rule.following_node_types.contains(&current_node.kind().to_string());
...
pub trait WithSemanticGrouping: SemanticGrouping {
    ...
    fn calculate_replacement_range<'a>(&self, tree: &'a Tree, node: Node<'a>, replacement_content: &str, _source_code: &str) -> Result<(usize, usize)> {
        ...
        let replacement_has_attributes = self.replacement_has_preceding_elements(replacement_content);
        ...
        if replacement_has_attributes && group.has_preceding_elements() {
            ...
            Ok((group.primary_node.start_byte(), group.primary_node.end_byte()))
        } else if replacement_has_attributes && !group.has_preceding_elements() {
    ...
    fn calculate_insertion_range<'a>(&self, tree: &'a Tree, node: Node<'a>, insert_before: bool) -> Result<(usize, usize)> {
        let group = self.find_semantic_group(tree, node)?;
        
        if insert_before {
            // Insert before the entire group (including any preceding elements)
            let start = group.preceding_nodes.first()
                .map(|n| n.start_byte())
                .unwrap_or(group.primary_node.start_byte());
            Ok((start, start))
        } else {
            // Insert after the entire group (including any following elements)
            let end = group.following_nodes.last()
                .map(|n| n.end_byte())
...
mod tests {
    ...
    fn test_grouping_rule_builder() {
        ...
        assert_eq!(rule.preceding_node_types, vec!["attribute_item", "line_comment"]);
