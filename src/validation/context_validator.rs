...
use tree_sitter::{Query, QueryCursor, Tree, Node, Language};
...
impl ContextValidator {
    ...
    fn load_validation_query(language: &str, lang_obj: &Language) -> Result<Query> {
        ...
        let query_source = match std::fs::read_to_string(format!("queries/{}/validation.scm", language)) {
            Ok(content) => content,
...
"#.to_string(),
    ...
    pub fn validate_insertion(
        ...
        let query = self.validation_queries.get(language)
            .ok_or_else(|| anyhow!("No validation rules for language: {}", language))?;
        
        // Simulate the insertion to create a temporary tree
        let temp_content = self.simulate_insertion(
            source_code, target_node, content, operation_type
        )?;
                ...
                if let Some(violation_type) = self.extract_violation_type(capture.index, query) {
                    ...
                    if violation_type.starts_with("invalid.") {
                        violations.push(ContextViolation {
                            ...
                            location: format!("{}:{}", node.start_position().row + 1, node.start_position().column + 1),
                            message: self.get_violation_message(&violation_type),
                            suggestion: self.get_violation_suggestion(&violation_type, &node, source_code),
                        });
                    }
                }
            }
    ...
    fn simulate_insertion(
        ...
        match operation_type {
            ...
            OperationType::InsertBefore => {
                ...
                temp_rope.insert(start_char, &format!("{}\n", content));
            }
    ...
    fn extract_violation_type(&self, capture_index: u32, query: &Query) -> Option<String> {
        query.capture_names().get(capture_index as usize).map(|s| s.to_string())
    }
    
    fn get_violation_message(&self, violation_type: &str) -> String {
        match violation_type {
            "invalid.function.in.struct.fields" => "Functions cannot be defined inside struct field lists".to_string(),
            "invalid.function.in.enum.variants" => "Functions cannot be defined inside enum variant lists".to_string(),
            "invalid.type.in.function.body" => "Type definitions cannot be placed inside function bodies".to_string(),
            "invalid.impl.in.function.body" => "Impl blocks cannot be placed inside function bodies".to_string(),
            "invalid.trait.in.function.body" => "Trait definitions cannot be placed inside function bodies".to_string(),
            "invalid.impl.nested" => "Impl blocks can only be defined at module level".to_string(),
            "invalid.trait.nested" => "Trait definitions can only be defined at module level".to_string(),
            "invalid.use.in.item.body" => "Use declarations should be at module level".to_string(),
            "invalid.const.in.function.body" => "Const/static items should be at module level".to_string(),
            "invalid.mod.in.function.body" => "Module declarations cannot be inside function bodies".to_string(),
            "invalid.item.nested.in.item" => "Items cannot be nested inside other items".to_string(),
            "invalid.expression.as.type" => "Expressions cannot be used as types".to_string(),
            _ => format!("Invalid placement: {}", violation_type.strip_prefix("invalid.").unwrap_or(violation_type)),
        }
    }
    
    fn get_violation_suggestion(&self, violation_type: &str, _node: &Node, _source_code: &str) -> ViolationSuggestion {
        match violation_type {
            "invalid.function.in.struct.fields" | "invalid.function.in.enum.variants" => ViolationSuggestion {
            ...
            "invalid.type.in.function.body" | "invalid.impl.in.function.body" | "invalid.trait.in.function.body" => ViolationSuggestion {
        ...
        for violation in &self.violations {
            ...
            if violation.suggestion.auto_correctable {
                if let Some(correction) = &violation.suggestion.corrected_operation {
                    response.push_str(&format!("  💡 Auto-correction available: Use {:?} operation instead.\n", 
